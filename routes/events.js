...
function eventsApi(app) {
 ...
 router.get('/search', async function (req, res, next) {
  try {
   const event = await eventsService.filterEvents(req.query);
   res.status(200).json({
    data: event,
    message: 'filtro',
   });
 ...
 router.get('/recents', async function (req, res, next) {
  try {
   const event = await eventsService.getEventsRecents();
   res.status(200).json({
    data: event,
    message: 'recents',
   });
 ...
 router.post(
  ...
  async function (req, res, next) {
   ...
   try {
    const createdEventId = await eventsService.createEvent({
     event,
    });
 ...
 router.put(
  ...
  async function (req, res, next) {
   ...
   const { eventId } = req.params;
   try {
    const updatedEventId = await eventsService.updateEvent({
     eventId,
     event,
    });
 ...
 router.delete(
  ...
  async function (req, res, next) {
   const { eventId } = req.params;
   try {
    const deletedEventId = await eventsService.deleteEvent({
     eventId,
    });
